export default class Util {
  ...
  static dateToString(date, onlyDate = false) {
    let ret = date.getFullYear() + '\\-' + Util.appendLeadingZeroes(date.getMonth() + 1) + '\\-' + Util.appendLeadingZeroes(date.getDate())
    if (!onlyDate) {
      ret += ' ' + Util.appendLeadingZeroes(date.getHours()) + ':' + Util.appendLeadingZeroes(date.getMinutes())
    }
    return ret
  }
